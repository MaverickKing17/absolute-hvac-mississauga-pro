  ...
  return (
    <div className="py-4">
      <div className="flex overflow-x-auto pb-8 gap-8 no-scrollbar snap-x">
        {reviews.map((rev, i) => (
          <div key={i} className="min-w-[340px] md:min-w-[420px] bg-white rounded-[2rem] shadow-2xl border border-slate-100 snap-center flex flex-col overflow-hidden group">
            ...
            <div className="bg-slate-50 border-b border-slate-100 p-6 flex justify-between items-center relative overflow-hidden">
               ...
               <div>
                 <div className="flex items-center gap-2 mb-1">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Class: {rev.sentiment}</span>
                 </div>
                 <h4 className="font-black text-trust-blue text-lg uppercase tracking-tighter">Report ID: #{(Math.random()*10000).toFixed(0)}</h4>
               </div>
               <div className="text-right">
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Conf. Score</p>
                 <p className="text-xl font-black text-emergency-orange leading-none">{rev.confidence}</p>
               </div>
            </div>
            ...
            <div className="p-8 flex-1 flex flex-col">
              ...
              <div className="relative">
                <p className="text-slate-600 text-lg leading-relaxed mb-8 italic relative z-10" dangerouslySetInnerHTML={{ __html: `"${rev.highlight}"` }}></p>
                <div className="absolute -top-4 -left-2 text-6xl text-slate-100 font-serif leading-none select-none">“</div>
              </div>
              ...
              <div className="mt-auto space-y-4">
                <div>
                  <div className="flex justify-between items-end mb-1.5">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 leading-none">Primary Keyword Intensity</span>
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl trust-blue text-white flex items-center justify-center font-black text-xl shadow-lg transform -rotate-3 group-hover:rotate-0 transition-transform">
                  {rev.author[0]}
                </div>
                <div>
                  <p className="font-black text-trust-blue uppercase tracking-tighter leading-none mb-1">{rev.author}</p>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{rev.location}, ON</p>
                </div>
              </div>
              <div className="w-10 h-10 border-2 border-slate-200 rounded-xl flex items-center justify-center text-slate-300">
