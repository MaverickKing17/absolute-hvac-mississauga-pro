...
const TestimonialCarousel: React.FC = () => {
  return (
    <div className="relative group">
      <div className="flex overflow-x-auto pb-10 gap-4 md:gap-6 no-scrollbar snap-x snap-mandatory cursor-grab active:cursor-grabbing px-4 -mx-4">
        {testimonials.map((t, i) => (
          <div 
            key={i} 
            className="min-w-[85vw] sm:min-w-[420px] bg-white p-6 md:p-10 rounded-[2.5rem] shadow-xl border border-slate-100 snap-center transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl flex flex-col"
          >
            <div className="text-trust-blue mb-4 md:mb-6">
              <svg className="w-10 h-10 md:w-12 md:h-12 opacity-20" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex text-yellow-400 mb-4 md:mb-6 gap-1">
              {[...Array(t.rating)].map((_, index) => (
                <svg key={index} className="w-4 h-4 md:w-5 md:h-5" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>

            <p className="text-slate-600 text-base md:text-lg leading-relaxed mb-6 md:mb-8 flex-grow italic">
              "{t.quote}"
            </p>

            <div className="flex items-center gap-4 pt-6 border-t border-slate-50">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl trust-blue text-white flex items-center justify-center font-black text-lg md:text-xl shadow-lg">
                {t.name[0]}
              </div>
              <div>
                <p className="font-black text-trust-blue text-base md:text-lg leading-none mb-1">{t.name}</p>
                <div className="flex items-center gap-1.5">
                  <svg className="w-3 h-3 text-emergency-orange" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                  </svg>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t.location}, ON</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="flex justify-center gap-2 mt-4 md:hidden">
        <div className="h-1 w-24 bg-trust-blue/10 rounded-full overflow-hidden">
